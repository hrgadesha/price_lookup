...
def get_price_history(item_code = None, history_based_on = None, party = None):
    record_limit = cint(frappe.db.get_single_value('Price Lookup Settings', 'supplier_quotation_historical_data_limit')) or 3
    ...
    sup_quote_item_details = frappe.db.sql("""
        ...
        where 
            ...
            sup_qtn.status not in ('Expired', 'Lost') {0}
        order by 
            sup_qtn.name desc
